...
void Group::assign(int narg, char **arg)
{
  ...
  double rlocal = n;
  // auto subreduce_check = [=](int n){
  //   MPI_Comm subcomm;
  //   MPI_Comm_split(MPI_COMM_WORLD, comm->me / n, comm->me % n, &subcomm);
  //   double all;
  //   MPI_Allreduce(&rlocal, &all, 1, MPI_DOUBLE, MPI_SUM, subcomm);
  //   if (comm->me % n == 0){
  //     printf("Allreduce result between %d and %d is %g\n", comm->me, comm->me + n, all);
  //   }
  // };
  // auto subreduce_check_strided = [=](int n){
  //   MPI_Comm subcomm;
  //   MPI_Comm_split(MPI_COMM_WORLD, comm->me % n, comm->me / n, &subcomm);
  //   double all;
  //   MPI_Allreduce(&rlocal, &all, 1, MPI_DOUBLE, MPI_SUM, subcomm);
  //   if (comm->me / n == 0){
  //     printf("Allreduce start %d stride %d is %g\n", comm->me, n, all);
  //   }
  // };

  // if (!strcmp(names[igroup], "satted")) {
  //   printf("proc %d has %g, %d atoms in satted\n", comm->me, rlocal, n);
  //   subreduce_check(32);
  //   subreduce_check(32);
  //   subreduce_check(32);
  //   subreduce_check(1024);
  //   subreduce_check(1024);
  //   subreduce_check(1024);
  //   subreduce_check(32768);
  //   subreduce_check(32768);
  //   subreduce_check(32768);
  //   subreduce_check(49152);
  //   subreduce_check(49152);
  //   subreduce_check(49152);
  //   subreduce_check(98304);
  //   subreduce_check(98304);
  //   subreduce_check(98304);
  //   subreduce_check_strided(32);
  //   subreduce_check_strided(32);
  //   subreduce_check_strided(32);
  //   subreduce_check_strided(1024);
  //   subreduce_check_strided(16384);
  //   subreduce_check_strided(49152);
  // }
...
void Group::xcmone(int igroup, double masstotal, double *cmone)
{
  int groupbit = bitmask[igroup];
...
void Group::vcmone(int igroup, double masstotal, double *cmone)
{
  int groupbit = bitmask[igroup];
  ...
  if (rmass) {
    for (int i = 0; i < nlocal; i++)
      if (mask[i] & groupbit) {
        massone = rmass[i];
        cmone[0] += v[i][0]*massone;
        cmone[1] += v[i][1]*massone;
        cmone[2] += v[i][2]*massone;
      }
  } else {
    for (int i = 0; i < nlocal; i++)
      if (mask[i] & groupbit) {
...
void Group::angmom_one(int igroup, double *cm, double *lmom)
{
  int groupbit = bitmask[igroup];
  ...
  for (int i = 0; i < nlocal; i++)
    if (mask[i] & groupbit) {
      ...
      lmom[0] += massone * (dy*v[i][2] - dz*v[i][1]);
      lmom[1] += massone * (dz*v[i][0] - dx*v[i][2]);
      lmom[2] += massone * (dx*v[i][1] - dy*v[i][0]);
    }
}
...
void Group::inertia_one(int igroup, double *cm, double ione[3][3])
{
  int i,j;
